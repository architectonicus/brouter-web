class TrackProfileGraph {
    ...
    _initUI(data) {
        ...
        const cumulAltScale = d3
            .scaleLinear()
            .domain(d3.extent(data, (d) => d.cumulAlt))
            .nice()
            .range([this._box.height - this._box.marginBottom, this._box.marginTop]);
        ...
        for (let i = 1; i < data.length; i++) {
            // for each point and the next, create a poly
        ...
        if (this._lineColorAttribute === 'altitude' || this._lineColorAttribute === 'cumulativeAltitude') {
            const cumulAltScale = d3
                .scaleLinear()
                .domain(d3.extent(data, (d) => d.cumulAlt))
                .nice()
        ...
        svg.on('touchstart touchmove', (e) => console.log(e));
        d3.drag((e) => console.log(e));
    ...
    _handleMouseMove(ev) {
        ...
        if (segmentix) {
            ...
            const alt =
                ((distanceFromStart - this._data[ix].cumulDist) * (this._data[ix + 1].alt - this._data[ix].alt)) /
                    (this._data[ix + 1].cumulDist - this._data[ix].cumulDist) +
                this._data[ix].alt;

            const pxy = y(alt);

            this._updateCursor(coords[0], pxy);
            this._updatePinPoint(distanceFromStart, alt, this._data[ix], [ev.layerX, 150]);
        }
    }
    ...
    _updatePinPoint(distanceFromStart, altitude, data, coords) {
        ...
        if (pinPointStatsView.css('visibility') === 'hidden') {
            pinPointStatsView.css('visibility', 'visible');
        }

        pinPointStatsView.css('left', coords[0]);
        //.css('bottom', coords[1]); // in styles.css
        pinPointStatsView.find('[data-distance]').html(distanceFromStart.toFixed(2));
        pinPointStatsView.find('[data-time]').html('12:34');
        pinPointStatsView.find('[data-gradient]').html(data.gradient.toFixed(0));
        pinPointStatsView.find('[data-altitude]').html(altitude.toFixed(0));
        pinPointStatsView
            .find('[data-cumulAlt]')
            .html(
                data.cumulAlt.toFixed(0) +
                    ' (' +
                    ((100 * data.cumulAlt) / this._data[this._data.length - 1].cumulAlt).toFixed(0) +
                    '%)'
            );
    ...
    _updateCursor(x, y) {
        ...
        d3.select(this._svgElementName)
            .select('svg')
            .append('circle')
